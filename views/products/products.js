...
/** Cache de todas las etiquetas del sistema (se invalida al cerrar el formulario de producto) */
let productFormAllTagsCache = null;
...
let productsRoleFilter = []; // Selected roles: 'vendible'|'comprable'|'insumo'|'producible'
let productsTagsFilter = []; // Selected tags for filter (product must have at least one)
// 'sin' = solo padres sin variantes, 'con' = con variantes (padres con variantes expandidos), 'todos' = padres + variantes en lista plana
let productsVariantsMode = 'todos';
...
// Load products using API with withVariants option
async function loadProducts() {
  ...
  productsList.innerHTML = '<div class="text-center py-8 text-gray-500">Cargando...</div>';

  const nrd = window.nrd;
  if (!nrd) {
    logger.error('NRD service not available');
    productsList.innerHTML = '<p class="text-center text-red-600 py-6">Servicio no disponible</p>';
    return;
  }

  try {
    let productsArray = [];
    if (productsVariantsMode === 'todos') {
      const flat = await nrd.products.getAll({ flat: true });
      productsArray = Array.isArray(flat) ? flat : Object.values(flat || {});
    } else if (productsVariantsMode === 'con') {
      ...
      productsArray = [...(withoutVariants || []), ...(withVariants || [])];
    } else {
      const all = await nrd.products.getAll();
      productsArray = Array.isArray(all) ? all : Object.values(all || {});
    }
    ...
    let productsToShow = productsArray.filter(p => p && (p.id || p.productId)).map(p => {
      const id = p.variantId ? `${p.productId}_${p.variantId}` : (p.id || p.productId);
      return [id, p];
    });
    ...
    if (productsSearchTerm.trim()) {
      ...
      productsToShow = productsToShow.filter(([id, product]) => {
        ...
        return name.includes(normalizedTerm) || productName.includes(normalizedTerm) ||
               sku.includes(normalizedTerm) || price.includes(normalizedTerm) ||
               priceFormatted.includes(normalizedTerm);
      });
    }

    if (productsRoleFilter.length > 0) {
      productsToShow = productsToShow.filter(([id, product]) => {
        return productsRoleFilter.some(role => {
          if (role === 'vendible') return product.esVendible === true;
          if (role === 'comprable') return product.esComprable === true;
          if (role === 'insumo') return product.esInsumo === true;
          if (role === 'producible') return product.esProducible === true;
          return false;
        });
      });
    }

    if (productsTagsFilter.length > 0) {
      productsToShow = productsToShow.filter(([id, product]) => {
        const productTags = normalizeTagsFromProduct(product).map(t => String(t).trim()).filter(Boolean);
        return productsTagsFilter.some(selectedTag => productTags.includes(selectedTag));
      });
    }

    if (productsToShow.length === 0) {
      productsList.innerHTML = '<p class="text-center text-gray-600 py-6 sm:py-8 text-sm sm:text-base">No hay productos que coincidan con los filtros</p>';
      return;
    }
    ...
    try {
      ...
      suppliers.forEach(s => { supplierNamesMap[s.id] = s.name; });
    } catch (e) {
      logger.warn('Could not load suppliers for product list', e);
    }

    productsToShow.forEach(([id, product]) => {
      const isVariant = !!product.variantId;
      const productId = product.productId || product.id;
      const displayName = isVariant && product.productName
        ? `${product.productName} - ${product.name}`
        : product.name;
      const hasVariants = !isVariant && product.variants && (Array.isArray(product.variants) ? product.variants.length > 0 : Object.keys(product.variants || {}).length > 0);
      const variantCount = hasVariants ? (Array.isArray(product.variants) ? product.variants.filter(v => v.active !== false).length : Object.values(product.variants || {}).filter(v => v.active !== false).length) : 0;
      const esVendible = product.esVendible === true;
      const esComprable = product.esComprable === true;
      const esInsumo = product.esInsumo === true;
      const esProducible = product.esProducible === true;
      const unitEntries = [];
      if (product.unidadVenta && product.unidadVenta.trim()) unitEntries.push({ type: 'Venta', unit: product.unidadVenta.trim() });
      if (product.unidadProduccion && product.unidadProduccion.trim()) unitEntries.push({ type: 'Producción', unit: product.unidadProduccion.trim() });
      if (product.unidadInsumo && product.unidadInsumo.trim()) unitEntries.push({ type: 'Insumo', unit: product.unidadInsumo.trim() });
      if (product.unidadesCompra && Array.isArray(product.unidadesCompra)) {
        product.unidadesCompra.forEach(pu => { if (pu && pu.unidad && pu.unidad.trim()) unitEntries.push({ type: 'Compra', unit: pu.unidad.trim() }); });
      }
      const seen = new Set();
      const uniqueUnitEntries = unitEntries.filter(({ type, unit }) => {
        const key = `${type}:${unit}`;
        if (seen.has(key)) return false;
        seen.add(key);
        return true;
      });
      const unitsLineHtml = uniqueUnitEntries.length > 0
        ? `<div class="mb-1 flex flex-wrap items-center gap-1.5 text-xs sm:text-sm text-gray-600">
          <span>Unidades de Medida:</span>
          ${uniqueUnitEntries.map(({ type, unit }) => `<span class="px-1.5 py-0.5 bg-gray-100 text-gray-700 rounded border border-gray-300 text-xs">${escapeHtml(type)}: ${escapeHtml(unit)}</span>`).join('')}
        </div>`
        : '';
      const tagsArr = normalizeTagsFromProduct(product);
      ...
      item.innerHTML = `
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0 mb-2 sm:mb-3">
          <div class="text-base sm:text-lg font-light flex-1">
            ...
            ${isVariant ? '<span class="ml-2 text-xs text-gray-500">(variante)</span>' : ''}
            ${hasVariants ? `<span class="ml-2 text-xs text-gray-500">(${variantCount} variante${variantCount !== 1 ? 's' : ''})</span>` : ''}
          </div>
          <span class="px-2 sm:px-3 py-0.5 sm:py-1 text-xs uppercase tracking-wider border ${product.active !== false ? 'border-red-600 text-red-600' : 'border-gray-300 text-gray-600'}">
        ...
        <div class="text-xs sm:text-sm text-gray-600">
          ...
          ${hasVariants ? `
          <div class="mb-1">Precio base: $${parseFloat(product.price || 0).toFixed(2)}</div>
          ` : `
          ...
          ${(product.supplierId || product.supplierCode) ? `<div class="mb-1">${product.supplierId ? `Proveedor: ${escapeHtml(supplierNamesMap[product.supplierId] || product.supplierId)}` : ''}${product.supplierId && product.supplierCode ? ' · ' : ''}${product.supplierCode ? `Cód. proveedor: ${escapeHtml(product.supplierCode)}` : ''}</div>` : ''}
          ${unitsLineHtml}
        </div>
        ${tagsArr.length > 0 ? `
        <div class="mt-2 pt-2 text-xs sm:text-sm text-gray-600 flex flex-wrap items-center gap-1">
          <span>Etiquetas:</span>
          ${tagsArr.map(t => `<span class="px-0.5 py-px text-[10px] text-amber-700 bg-amber-50 rounded border border-amber-200 font-normal">${escapeHtml(String(t).trim())}</span>`).join('')}
        </div>
        ` : ''}
        ${(esVendible || esComprable || esInsumo || esProducible) ? `
        <div class="mt-2 pt-2 border-t border-gray-200">
          <div class="flex flex-wrap gap-1">
            ${esVendible ? '<span class="px-1.5 py-0.5 bg-green-100 text-green-700 rounded border border-green-300 text-xs">Vendible</span>' : ''}
            ${esComprable ? '<span class="px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded border border-blue-300 text-xs">Comprable</span>' : ''}
            ${esInsumo ? '<span class="px-1.5 py-0.5 bg-orange-100 text-orange-700 rounded border border-orange-300 text-xs">Insumo</span>' : ''}
            ${esProducible ? '<span class="px-1.5 py-0.5 bg-purple-100 text-purple-700 rounded border border-purple-300 text-xs">Producible</span>' : ''}
          </div>
        </div>
        ` : ''}
      `;
      item.addEventListener('click', () => viewProduct(productId));
      productsList.appendChild(item);
    });
  } catch (error) {
    logger.error('Error loading products', error);
    if (productsList) {
      productsList.innerHTML = '<p class="text-center text-red-600 py-6">Error al cargar productos</p>';
    }
  }
}

// --- Tags (etiquetas) en formulario de producto ---
function normalizeTagsFromProduct(product) {
  const t = product && product.tags;
  if (!t) return [];
  return Array.isArray(t) ? t : (typeof t === 'object' && t !== null ? Object.values(t) : []);
}

function renderProductTags(tagsArray) {
  ...
  list.forEach(tag => {
    ...
    chip.className = 'inline-flex items-center gap-0.5 px-2 py-0.5 rounded border border-gray-300 bg-gray-100 text-gray-800 text-xs shrink-0';
    chip.dataset.tag = s;
    chip.innerHTML = `${escapeHtml(s)} <button type="button" class="product-tag-remove ml-0.5 text-gray-500 hover:text-red-600 font-medium leading-none" aria-label="Quitar">×</button>`;
    container.appendChild(chip);
  });
}

function getProductTagsFromForm() {
  const container = document.getElementById('product-tags-container');
  if (!container) return [];
  const chips = container.querySelectorAll('[data-tag]');
  return Array.from(chips).map(c => (c.dataset.tag || '').trim()).filter(Boolean);
}

/** Normaliza texto a etiqueta: trim, varias palabras unidas con _ */
function normalizeTagText(tag) {
  const s = String(tag).trim().replace(/\s+/g, '_');
  return s ? s.toUpperCase() : '';
}

function addProductTag(tag) {
  const s = normalizeTagText(tag);
  if (!s) return;
  const current = getProductTagsFromForm();
  if (current.includes(s)) return;
  renderProductTags([...current, s]);
...
async function showProductForm(productId = null) {
  ...
  try {
    ...
    if (productSupplierSelect) {
      productSupplierSelect.innerHTML = '<option value="">Ninguno</option>' +
        suppliers.map(s => `<option value="${escapeHtml(s.id)}">${escapeHtml(s.name)}</option>`).join('');
    }
  } catch (err) {
    logger.error('Failed to load suppliers for product form', err);
  }
  renderProductTags([]);
  ...
  const deleteBtn = document.getElementById('delete-product-form-btn');
  ...
  if (productId) {
    ...
    // Mostrar botón eliminar en rojo
    if (deleteBtn) {
      deleteBtn.classList.remove('hidden');
      // Asegurar que el botón tenga type="button" para evitar submit del formulario
      deleteBtn.type = 'button';
      // Configurar el event listener del botón de eliminar cada vez que se muestra el formulario
      // Remover listeners anteriores clonando el botón
      const newDeleteBtn = deleteBtn.cloneNode(true);
      newDeleteBtn.type = 'button'; // Asegurar que el tipo se mantenga después de clonar
      ...
      newDeleteBtn.addEventListener('click', async (e) => {
        ...
        if (currentProductId) {
          await deleteProductHandler(currentProductId);
        }
      });
    }
    ...
    (async () => {
      ...
      if (product) {
        ...
        const productSupplierEl = document.getElementById('product-supplier');
        const productSupplierCodeEl = document.getElementById('product-supplier-code');
        if (productSupplierEl) productSupplierEl.value = product.supplierId || '';
        if (productSupplierCodeEl) productSupplierCodeEl.value = product.supplierCode || '';
        
        // Load roles
        // Si tiene variantes, el producto padre no puede ser vendible
        const hasVariants = product.variants && product.variants.length > 0;
        const esVendibleCheckbox = document.getElementById('product-es-vendible');
        if (esVendibleCheckbox) {
          if (hasVariants) {
            esVendibleCheckbox.checked = false;
            esVendibleCheckbox.disabled = true;
          } else {
            esVendibleCheckbox.checked = product.esVendible === true;
            esVendibleCheckbox.disabled = false;
          }
        }
        ...
        // Load tags
        renderProductTags(normalizeTagsFromProduct(product));
        ...
        if (variantsList) {
          ...
          // Actualizar estado del checkbox después de cargar variantes
          updateVariantsEmptyState();
        }
      }
    })();
  } else {
    // Ocultar botón eliminar para productos nuevos
    if (deleteBtn) {
...
async function viewProduct(productId) {
  ...
  try {
    ...
    // Load supplier names for purchase units and main product supplier
    const supplierNamesMap = {};
    if (product.supplierId || (product.unidadesCompra && product.unidadesCompra.length > 0)) {
      try {
        const suppliers = await loadSuppliers();
    ...
    document.getElementById('product-detail-content').innerHTML = `
      <div class="space-y-3 sm:space-y-4">
        ...
        <div class="flex justify-between py-2 sm:py-3 border-b border-gray-200">
          ...
          <span class="font-light text-sm sm:text-base">${escapeHtml(supplierNamesMap[product.supplierId] || product.supplierId)}</span>
        </div>
        ` : ''}
        ${product.supplierCode ? `
        <div class="flex justify-between py-2 sm:py-3 border-b border-gray-200">
          <span class="text-gray-600 font-light text-sm sm:text-base">Código en proveedor:</span>
          <span class="font-light text-sm sm:text-base font-mono">${escapeHtml(product.supplierCode)}</span>
        </div>
        ` : ''}
        ${(() => {
          const tagsArr = normalizeTagsFromProduct(product);
          if (tagsArr.length === 0) return '';
          const badges = tagsArr.map(t => `<span class="px-0.5 py-px text-[10px] text-amber-700 bg-amber-50 rounded border border-amber-200 font-normal">${escapeHtml(String(t).trim())}</span>`).join('');
          return `
        <div class="py-2 sm:py-3 -mt-px pt-0 border-b border-gray-200 bg-white">
          <span class="text-gray-600 font-light text-sm sm:text-base">Etiquetas:</span>
          <span class="ml-1.5 inline-flex flex-wrap gap-1 align-middle">${badges}</span>
        </div>
        `;
        })()}
        ...
        ` : ''}
        ${(product.unidadVenta || product.unidadProduccion || (product.unidadesCompra && product.unidadesCompra.length > 0) || product.unidadInsumo) ? (() => {
          const entries = [];
          if (product.unidadVenta && product.unidadVenta.trim()) entries.push({ type: 'Venta', unit: product.unidadVenta.trim() });
          if (product.unidadProduccion && product.unidadProduccion.trim()) entries.push({ type: 'Producción', unit: product.unidadProduccion.trim() });
          if (product.unidadInsumo && product.unidadInsumo.trim()) entries.push({ type: 'Insumo', unit: product.unidadInsumo.trim() });
          if (product.unidadesCompra && Array.isArray(product.unidadesCompra)) product.unidadesCompra.forEach(pu => { if (pu && pu.unidad && pu.unidad.trim()) entries.push({ type: 'Compra', unit: pu.unidad.trim() }); });
          const seen = new Set();
          const unique = entries.filter(({ type, unit }) => { const k = `${type}:${unit}`; if (seen.has(k)) return false; seen.add(k); return true; });
          return `
        <div class="py-2 sm:py-3 border-b border-gray-200">
          <div class="text-gray-600 font-light text-sm sm:text-base mb-2">Unidades de Medida:</div>
          <div class="flex flex-wrap gap-1">
            ${unique.map(({ type, unit }) => `<span class="px-2 py-1 bg-gray-100 text-gray-700 rounded border border-gray-300 text-xs">${escapeHtml(type)}: ${escapeHtml(unit)}</span>`).join('')}
          </div>
        </div>
        `;
        })() : ''}
        ...
        <div class="py-2 sm:py-3 border-b border-gray-200">
          ...
          <div class="space-y-2">
            ${product.variants.map(variant => {
              ...
              const unidadInsumo = variant.unidadInsumo || product.unidadInsumo;
              ...
              return `
                <div class="bg-gray-50 p-2 sm:p-3 rounded border border-gray-200">
                  ...
                  ` : ''}
                  ${(unidadVenta || unidadProduccion || (unidadesCompra && unidadesCompra.length > 0) || unidadInsumo) ? (() => {
                    const uv = [];
                    if (unidadVenta && unidadVenta.trim()) uv.push({ type: 'Venta', unit: unidadVenta.trim() });
                    if (unidadProduccion && unidadProduccion.trim()) uv.push({ type: 'Producción', unit: unidadProduccion.trim() });
                    if (unidadInsumo && unidadInsumo.trim()) uv.push({ type: 'Insumo', unit: unidadInsumo.trim() });
                    if (unidadesCompra && Array.isArray(unidadesCompra)) unidadesCompra.forEach(pu => { if (pu && pu.unidad && pu.unidad.trim()) uv.push({ type: 'Compra', unit: pu.unidad.trim() }); });
                    const seen = new Set();
                    const uniqueV = uv.filter(({ type, unit }) => { const k = `${type}:${unit}`; if (seen.has(k)) return false; seen.add(k); return true; });
                    return `
                  <div class="text-xs mt-2 pt-2 border-t border-gray-300">
                    <div class="text-gray-600 font-medium mb-1">Unidades de medida:</div>
                    <div class="flex flex-wrap gap-1">
                      ${uniqueV.map(({ type, unit }) => `<span class="px-1.5 py-0.5 bg-gray-100 text-gray-700 rounded border border-gray-300">${escapeHtml(type)}: ${escapeHtml(unit)}</span>`).join('')}
                    </div>
                  </div>
                  `;
                  })() : ''}
...
function addVariantRow(variant = null) {
  ...
  // Get parent product price to calculate plus
  let parentPrice = 0;
  let pricePlus = 0;
  if (variant) {
    // Try to get parent product price from the form
    const parentPriceInput = document.getElementById('product-price');
    if (parentPriceInput) {
      parentPrice = parseFloat(parentPriceInput.value) || 0;
      pricePlus = parseFloat(variant.price || 0) - parentPrice;
    }
  }
    ...
    <div class="flex items-center justify-between">
      <div class="flex-1">
        ...
        <div class="text-xs text-gray-600 space-x-3">
          ...
          ${pricePlus !== 0 ? `<span class="text-gray-500">(Plus: ${pricePlus >= 0 ? '+' : ''}$${pricePlus.toFixed(2)})</span>` : ''}
...
function updateVariantsEmptyState() {
  ...
  // Actualizar estado del checkbox esVendible del producto padre
  const esVendibleCheckbox = document.getElementById('product-es-vendible');
  if (esVendibleCheckbox) {
    if (hasVariants) {
      // Si tiene variantes, deshabilitar y desmarcar el checkbox
      esVendibleCheckbox.disabled = true;
      esVendibleCheckbox.checked = false;
      // Agregar tooltip o mensaje informativo
      const label = esVendibleCheckbox.closest('label');
      if (label && !label.querySelector('.variant-info')) {
        const info = document.createElement('span');
        info.className = 'variant-info text-xs text-gray-500 italic ml-2';
        info.textContent = '(No vendible cuando tiene variantes)';
        label.appendChild(info);
      }
    } else {
      // Si no tiene variantes, habilitar el checkbox
      esVendibleCheckbox.disabled = false;
      // Remover mensaje informativo si existe
      const label = esVendibleCheckbox.closest('label');
      if (label) {
        const info = label.querySelector('.variant-info');
        if (info) {
          info.remove();
        }
      }
...
async function getUnitOptionsHTMLForProduct(selectedUnit = '', forceReload = true) {
  // Force reload to get latest units from database
  const units = await loadMeasurementUnits(forceReload);
  
  // If no units found, return empty or selected unit if exists
  if (units.length === 0) {
    logger.warn('No measurement units found in database');
    return selectedUnit ? `<option value="${escapeHtml(selectedUnit)}" selected>${escapeHtml(selectedUnit)}</option>` : '<option value="">No hay unidades disponibles</option>';
  }
  
  logger.debug('Generating unit options HTML', { unitsCount: units.length, selectedUnit });
  
  return units.map(unit => {
    if (!unit || !unit.name) {
      logger.warn('Invalid unit found', unit);
      return '';
    }
    return `<option value="${escapeHtml(unit.name)}" ${selectedUnit === unit.name ? 'selected' : ''}>${escapeHtml(unit.name)}</option>`;
  }).filter(opt => opt).join('');
}

// Get current product/variant roles from form (product form or variant modal)
function getProductRolesForUnitModal() {
  const isVariantContext = !!window.currentVariantRowForUnits;
  return {
    esVendible: isVariantContext
      ? document.getElementById('modal-variant-es-vendible')?.checked === true
      : document.getElementById('product-es-vendible')?.checked === true,
    esComprable: isVariantContext
      ? document.getElementById('modal-variant-es-comprable')?.checked === true
      : document.getElementById('product-es-comprable')?.checked === true,
    esProducible: isVariantContext
      ? document.getElementById('modal-variant-es-producible')?.checked === true
      : document.getElementById('product-es-producible')?.checked === true,
    esInsumo: isVariantContext
      ? document.getElementById('modal-variant-es-insumo')?.checked === true
      : document.getElementById('product-es-insumo')?.checked === true
  };
}

// Build type options for unit modal based on product roles (venta→vendible, compra→comprable, produccion→producible, insumo→insumo)
function getUnitTypeOptionsForRoles(roles) {
  const options = [];
  if (roles.esVendible) options.push({ value: 'venta', label: 'Venta' });
  if (roles.esComprable) options.push({ value: 'compra', label: 'Compra' });
  if (roles.esProducible) options.push({ value: 'produccion', label: 'Producción' });
  if (roles.esInsumo) options.push({ value: 'insumo', label: 'Insumo' });
  return options;
}
...
async function showEditProductUnitModal(unitData = null, unitKey = null) {
  ...
  // Get roles to filter unit types (solo mostrar tipos según roles del producto)
  const roles = getProductRolesForUnitModal();
  const typeOptions = getUnitTypeOptionsForRoles(roles);
  
  const isEditing = unitData !== null;
  let selectedType = unitData ? unitData.type : '';
  const selectedUnit = unitData ? (unitData.unitName || '') : '';
  const selectedSupplier = unitData && unitData.supplierId ? unitData.supplierId : '';
  
  // Si el tipo actual no está permitido por roles, usar el primero disponible
  const allowedTypes = typeOptions.map(o => o.value);
  if (selectedType && !allowedTypes.includes(selectedType)) {
    selectedType = allowedTypes.length > 0 ? allowedTypes[0] : '';
  }
  if (!selectedType && allowedTypes.length > 0) {
  ...
  // Load units from database (force reload to get latest)
  let unitOptionsHTML = '';
  try {
    // Verify nrd service is available
    const nrd = window.nrd;
    if (!nrd) {
      logger.error('NRD service not available');
      unitOptionsHTML = '<option value="">Error: Servicio NRD no disponible</option>';
    } else if (!nrd.measurementUnits) {
      logger.error('MeasurementUnits service not available', {
        nrdAvailable: true,
        measurementUnitsAvailable: false,
        availableServices: Object.keys(nrd).filter(k => k !== 'auth')
      });
      unitOptionsHTML = '<option value="">Error: Servicio de unidades de medida no disponible</option>';
    } else {
      unitOptionsHTML = await getUnitOptionsHTMLForProduct(selectedUnit, true);
      if (!unitOptionsHTML || unitOptionsHTML.trim() === '') {
        logger.warn('No unit options generated, units may be empty');
        unitOptionsHTML = '<option value="">No hay unidades disponibles. Cree unidades en la gestión de medidas.</option>';
      }
    }
  } catch (error) {
    logger.error('Error loading unit options', error);
    unitOptionsHTML = '<option value="">Error al cargar unidades de medida</option>';
  }
  
  const typeOptionsHTML = typeOptions.length === 0
    ? '<option value="">Seleccione al menos un rol del producto (Vendible, Comprable, Producible o Insumo)</option>'
    : typeOptions.map(opt => `<option value="${opt.value}" ${selectedType === opt.value ? 'selected' : ''}>${escapeHtml(opt.label)}</option>`).join('');
    ...
    <form id="add-product-unit-form" class="space-y-4">
      <div>
        ...
        <select id="modal-unit-type" required
          ...
          ${typeOptions.length === 0 ? typeOptionsHTML : '<option value="">Seleccione...</option>' + typeOptionsHTML}
        </select>
        ${typeOptions.length > 0 ? '<p class="text-xs text-gray-500 mt-1">Solo se muestran tipos según los roles del producto (Vendible→Venta, Comprable→Compra, Producible→Producción, Insumo→Insumo).</p>' : ''}
      </div>
  ...
  const handleConfirm = async () => {
    if (typeOptions.length === 0) {
      ...
      showErrorFn('Seleccione al menos un rol del producto (Vendible, Comprable, Producible o Insumo) antes de agregar unidades de medida.');
      return;
    }
...
async function showEditVariantModal(variantData = null, variantKey = null) {
  ...
  // Get parent product data (needed for both create and edit)
  ...
  if (productId) {
    ...
    if (nrd && nrd.products) {
      try {
        parentProduct = await nrd.products.getById(productId);
        if (parentProduct) {
          parentProductName = parentProduct.name || '';
        }
      } catch (error) {
        logger.warn('Error loading parent product for variant', error);
      }
    }
  }
  // SKU por defecto: siempre padre + consecutivo (al crear variante). Considera variantes guardadas y las ya agregadas en el formulario.
  if (!isEditing) {
    const parentSku = (parentProduct && parentProduct.sku) ? String(parentProduct.sku).trim() : (document.getElementById('product-sku')?.value?.trim() || '');
    if (parentSku) {
      const skuPattern = new RegExp(`^${parentSku.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}_(\\d+)$`, 'i');
      let maxConsecutive = 0;
      // Variantes guardadas del producto
      const savedVariants = parentProduct?.variants || [];
      savedVariants.forEach(v => {
        if (v && v.sku) {
          const m = String(v.sku).match(skuPattern);
          if (m) {
            const n = parseInt(m[1], 10);
            if (n > maxConsecutive) maxConsecutive = n;
          }
        }
      });
      // Variantes ya agregadas en el formulario (aún no guardadas)
      const variantsList = document.getElementById('product-variants-list');
      if (variantsList && variantsList.children) {
        for (const row of variantsList.children) {
          const data = row.dataset.variantData;
          if (!data) continue;
          try {
      ...
      defaultSku = `${parentSku}_${nextConsecutive}`;
    }
  }
  
  // Inherit values from parent product when creating new variant
  const variantName = variantData ? escapeHtml(variantData.name || '') : '';
  const variantSku = variantData ? escapeHtml(variantData.sku || '') : defaultSku;
  
  // Calculate price plus (difference from parent price)
  const parentPrice = parentProduct?.price ? parseFloat(parentProduct.price) : 0;
  let variantPricePlus = 0;
  if (variantData && variantData.price !== undefined) {
    // If editing, calculate plus = variant price - parent price
    const variantPrice = parseFloat(variantData.price || 0);
    variantPricePlus = variantPrice - parentPrice;
  }
  
  const variantPricePlusStr = variantPricePlus.toFixed(2);
  const parentPriceStr = parentPrice.toFixed(2);
  const calculatedPriceStr = (parentPrice + variantPricePlus).toFixed(2);
  
  const variantCost = variantData && variantData.cost !== undefined 
    ? parseFloat(variantData.cost).toFixed(2) 
    : (parentProduct?.cost !== undefined ? parseFloat(parentProduct.cost).toFixed(2) : '');
  const isActive = variantData ? (variantData.active !== false) : (parentProduct?.active !== false);
  const esVendible = variantData ? (variantData.esVendible === true) : (parentProduct?.esVendible === true);
  const esComprable = variantData ? (variantData.esComprable === true) : (parentProduct?.esComprable === true);
  const esInsumo = variantData ? (variantData.esInsumo === true) : (parentProduct?.esInsumo === true);
  const esProducible = variantData ? (variantData.esProducible === true) : (parentProduct?.esProducible === true);
  
  // Get parent product units to inherit
  const parentUnits = parentProduct && !isEditing ? (parentProduct.unidadVenta || parentProduct.unidadProduccion || parentProduct.unidadesCompra || []) : null;
  ...
  const formHTML = `
    <form id="add-variant-form" class="space-y-4 max-h-[80vh] overflow-y-auto">
      ...
      <div class="space-y-3">
        <div>
          <label class="block mb-1.5 text-xs uppercase tracking-wider text-gray-600">
            Nombre * ${parentProductName ? `<span class="text-xs font-normal text-gray-500 normal-case">(Producto padre: ${escapeHtml(parentProductName)})</span>` : ''}
          </label>
          <input type="text" id="modal-variant-name" value="${variantName}"
            class="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:border-red-600"
            placeholder="Ej: Pequeña, Grande, Chocolate" required
            data-parent-name="${escapeHtml(parentProductName)}">
        </div>
        <div>
        ...
        <div class="space-y-3">
          <div>
            <label class="block mb-1.5 text-xs uppercase tracking-wider text-gray-600">Precio del Producto Padre</label>
            <div class="px-3 py-2 border border-gray-300 rounded text-sm bg-gray-50 text-gray-700">
              $${parentPriceStr}
            </div>
          </div>
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block mb-1.5 text-xs uppercase tracking-wider text-gray-600">Plus sobre Precio Padre *</label>
              <input type="number" id="modal-variant-price-plus" step="0.01" value="${variantPricePlusStr}"
                class="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:border-red-600" 
                placeholder="0.00" required>
            </div>
            <div>
              <label class="block mb-1.5 text-xs uppercase tracking-wider text-gray-600">Precio Final Calculado</label>
              <div id="modal-variant-calculated-price" class="px-3 py-2 border border-gray-300 rounded text-sm bg-gray-50 text-red-600 font-medium">
                $${calculatedPriceStr}
              </div>
            </div>
          </div>
  ...
  // Load variant units if editing, or inherit from parent if creating new variant
  if (variantData) {
    await loadVariantUnitsToTable(variantData, window.currentVariantRowForUnits);
  } else if (parentProduct) {
    // Inherit units from parent product when creating new variant
    // Convert parent product to variant-like format for loadVariantUnitsToTable
    const parentAsVariant = {
      unidadVenta: parentProduct.unidadVenta,
      unidadProduccion: parentProduct.unidadProduccion,
      unidadesCompra: parentProduct.unidadesCompra,
      unidadInsumo: parentProduct.unidadInsumo,
      conversiones: parentProduct.conversiones
    };
    await loadVariantUnitsToTable(parentAsVariant, window.currentVariantRowForUnits);
  }
  ...
  // Update calculated price when plus changes
  const pricePlusInput = document.getElementById('modal-variant-price-plus');
  const calculatedPriceDiv = document.getElementById('modal-variant-calculated-price');
  if (pricePlusInput && calculatedPriceDiv) {
    const updateCalculatedPrice = () => {
      const plusValue = parseFloat(pricePlusInput.value) || 0;
      const finalPrice = parentPrice + plusValue;
      calculatedPriceDiv.textContent = `$${finalPrice.toFixed(2)}`;
    };
    pricePlusInput.addEventListener('input', updateCalculatedPrice);
  }
  ...
  const handleConfirm = async () => {
    ...
    const pricePlusValue = document.getElementById('modal-variant-price-plus')?.value.trim();
    ...
    // Validate that name is different from parent product name
    const parentNameAttr = document.getElementById('modal-variant-name')?.getAttribute('data-parent-name');
    if (parentNameAttr && name.toLowerCase().trim() === parentNameAttr.toLowerCase().trim()) {
      await showError(`El nombre de la variante debe ser diferente al nombre del producto padre: "${parentNameAttr}"`);
      return;
    }
    
    // Calculate final price = parent price + plus
    const pricePlus = parseFloat(pricePlusValue) || 0;
    if (isNaN(pricePlus)) {
      await showError('El plus sobre precio debe ser un número válido');
      return;
    }
    
    const price = parentPrice + pricePlus;
    if (price < 0) {
      await showError('El precio final no puede ser negativo');
      return;
    }
    ...
    variantUnits.forEach(unit => {
      ...
      } else if (unit.type === 'insumo') {
        unidadInsumo = unit.unitName;
      }
    });
    ...
    if (unidadInsumo) {
      newVariant.unidadInsumo = unidadInsumo;
    }
...
async function loadProductUnitsToTable(product) {
  ...
  // Add insumo unit
  if (product.unidadInsumo) {
    units.push({ type: 'insumo', unitName: product.unidadInsumo });
  }
...
async function loadVariantUnitsToTable(variant, variantRow) {
  ...
  // Add insumo unit
  if (variant.unidadInsumo) {
    units.push({ type: 'insumo', unitName: variant.unidadInsumo });
  }
...
async function loadMeasurementUnits(forceReload = false) {
  if (measurementUnitsCache && !forceReload) {
    logger.debug('Using cached measurement units', { count: measurementUnitsCache.length });
  ...
  try {
    ...
    logger.debug('Loading measurement units from database...');
    ...
    if (Array.isArray(units)) {
      ...
      unitsArray = Object.values(units);
    }
    
    // Filter out invalid units
    unitsArray = unitsArray.filter(u => u && u.name && u.name.trim() !== '');
    
    measurementUnitsCache = unitsArray;
    logger.info('Loaded measurement units', { 
      count: measurementUnitsCache.length,
      unitNames: measurementUnitsCache.map(u => u.name)
    });
    
    return measurementUnitsCache;
  } catch (error) {
    logger.error('Failed to load measurement units', error);
    measurementUnitsCache = null; // Clear cache on error
    return [];
  }
}
...
async function deleteProductHandler(productId) {
  ...
  try {
    ...
    if (logger.audit) {
      ...
      logger.info('Product deleted', { entity: 'product', id: productId, uid: user?.uid, email: user?.email, timestamp: Date.now() });
    }
...
// True si hay al menos una variante en el formulario con esVendible
function hasAnyVariantVendible() {
  const variantsList = document.getElementById('product-variants-list');
  if (!variantsList || !variantsList.children.length) return false;
  for (const row of variantsList.children) {
    const data = row.dataset.variantData;
    if (!data) continue;
    try {
      const v = JSON.parse(data);
      if (v && v.esVendible === true) return true;
    } catch (e) { /* ignore */ }
  }
  return false;
}

// Update price field: required and enabled when product is vendible, or when has variants and any variant is vendible
function updatePriceFieldState() {
  const esVendibleCheckbox = document.getElementById('product-es-vendible');
  const priceInput = document.getElementById('product-price');
  const priceLabel = document.getElementById('product-price-label');
  if (!esVendibleCheckbox || !priceInput) return;
  const esVendible = esVendibleCheckbox.checked;
  const vendibleDisabled = esVendibleCheckbox.disabled; // true cuando tiene variantes
  const anyVariantVendible = vendibleDisabled ? hasAnyVariantVendible() : false;
  const requirePrice = (esVendible && !vendibleDisabled) || (vendibleDisabled && anyVariantVendible);
  priceInput.required = requirePrice;
  priceInput.disabled = !requirePrice;
  if (priceInput.disabled) {
    priceInput.value = '';
    priceInput.classList.add('bg-gray-100', 'cursor-not-allowed');
    priceInput.classList.remove('bg-transparent');
  } else {
    priceInput.classList.remove('bg-gray-100', 'cursor-not-allowed');
    priceInput.classList.add('bg-transparent');
  }
  if (priceLabel) priceLabel.textContent = requirePrice ? 'Precio *' : 'Precio';
}
...
function setupProductFormHandler() {
  ...
  // Price field: enable/require only when "Es vendible" is checked
  const esVendibleCheckbox = document.getElementById('product-es-vendible');
  if (esVendibleCheckbox) {
    esVendibleCheckbox.addEventListener('change', updatePriceFieldState);
  }
  ...
  // Tags: al hacer foco se listan TODAS las etiquetas del sistema (todos los productos) para seleccionar
  const tagsWrapper = document.getElementById('product-tags-wrapper');
  const tagInput = document.getElementById('product-tag-input');
  const tagsDropdown = document.getElementById('product-tags-dropdown');
  let tagsDropdownHideTimeout = null;
  async function loadAllTagsFromProducts() {
    if (productFormAllTagsCache) return productFormAllTagsCache;
    const nrd = window.nrd;
    if (!nrd || !nrd.products) return [];
    try {
      const [withVariants, withoutVariants] = await Promise.all([
        nrd.products.getAll({ withVariants: true }),
        nrd.products.getAll({ withVariants: false })
      ...
      allItems.forEach(item => {
        normalizeTagsFromProduct(item).forEach(t => {
          const s = String(t).trim();
          if (s) set.add(s);
        });
      });
      productFormAllTagsCache = Array.from(set).sort((a, b) => a.localeCompare(b));
      return productFormAllTagsCache;
    } catch (e) {
      logger.warn('Error loading all tags', e);
      return [];
    }
  }
  async function showTagsDropdown() {
    if (!tagsDropdown) return;
    tagsDropdown.innerHTML = '<div class="px-3 py-2 text-xs text-gray-500">Cargando etiquetas...</div>';
    tagsDropdown.classList.remove('hidden');
    const current = getProductTagsFromForm();
    const dbTags = await loadAllTagsFromProducts();
    const allTags = [...new Set([...dbTags, ...current])].sort((a, b) => a.localeCompare(b));
    if (allTags.length === 0) {
      tagsDropdown.innerHTML = '<div class="px-3 py-2 text-xs text-gray-500">No hay etiquetas en el sistema. Escriba y pulse Enter para agregar una nueva.</div>';
    } else {
      tagsDropdown.innerHTML = allTags.map(t => {
        const added = current.includes(t);
        return `
        <button type="button" class="product-tags-dropdown-item w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-gray-100 flex items-center justify-between gap-2" data-tag="${escapeHtml(t)}" data-added="${added}">
          <span class="px-0.5 py-px text-[10px] text-amber-700 bg-amber-50 rounded border border-amber-200">${escapeHtml(t)}</span>
          <span class="text-gray-500">${added ? 'Clic para quitar' : 'Clic para agregar'}</span>
        </button>
      `;
      }).join('');
      tagsDropdown.querySelectorAll('.product-tags-dropdown-item').forEach(btn => {
        btn.addEventListener('click', (e) => {
          e.preventDefault();
          const tag = btn.dataset.tag;
          const isAdded = getProductTagsFromForm().includes(tag);
          if (isAdded) {
            const list = getProductTagsFromForm().filter(x => x !== tag);
            renderProductTags(list);
          } else {
            addProductTag(tag);
          }
          tagInput.focus();
  ...
  if (tagsWrapper && tagInput) {
    tagsWrapper.addEventListener('click', () => tagInput.focus());
    tagInput.addEventListener('focus', () => {
      if (tagsDropdownHideTimeout) {
        clearTimeout(tagsDropdownHideTimeout);
        tagsDropdownHideTimeout = null;
      }
      showTagsDropdown();
    });
    tagInput.addEventListener('blur', () => {
      tagsDropdownHideTimeout = setTimeout(hideTagsDropdown, 200);
    });
  ...
  if (tagsContainer) {
    tagsContainer.addEventListener('click', (e) => {
      const removeBtn = e.target.closest('.product-tag-remove');
      if (!removeBtn) return;
      e.preventDefault();
      e.stopPropagation();
      const chip = removeBtn.closest('[data-tag]');
      if (!chip) return;
      const current = getProductTagsFromForm();
      const toRemove = (chip.dataset.tag || '').trim();
      renderProductTags(current.filter(t => t !== toRemove));
    });
  }

  // Delete product button handler (in form)
  const deleteProductFormBtn = document.getElementById('delete-product-form-btn');
  if (deleteProductFormBtn) {
    // Asegurar que el botón tenga type="button" para evitar submit del formulario
    deleteProductFormBtn.type = 'button';
    const newDeleteBtn = deleteProductFormBtn.cloneNode(true);
    newDeleteBtn.type = 'button'; // Asegurar que el tipo se mantenga después de clonar
    deleteProductFormBtn.parentNode.replaceChild(newDeleteBtn, deleteProductFormBtn);
    newDeleteBtn.addEventListener('click', async (e) => {
      e.preventDefault();
      e.stopPropagation();
      const productId = document.getElementById('product-id')?.value;
  ...
  formElement.addEventListener('submit', async (e) => {
    ...
    const anyVariantVendible = hasVariants && variants.some(v => v.esVendible === true);
    const priceInput = document.getElementById('product-price');
    const requireParentPrice = esVendible || (hasVariants && anyVariantVendible);
    const price = requireParentPrice ? parseFloat(priceInput.value) : 0;
    const costInput = document.getElementById('product-cost');
    ...
    // Get roles (esVendible already read above)
    ...
    if (!name) {
      await showError('El nombre del producto es obligatorio');
      return;
    }
    if (requireParentPrice && (isNaN(price) || price < 0)) {
      await showError('El precio de venta es obligatorio y debe ser mayor o igual a 0 (producto vendible o variante vendible)');
      return;
    }
    ...
    // Validate variants (ya recolectadas arriba)
    ...
    const insumoUnit = productUnits.find(u => u.type === 'insumo');
    
    const unidadVenta = ventaUnit?.unitName || '';
    const unidadProduccion = produccionUnit?.unitName || '';
    const unidadInsumo = insumoUnit?.unitName || '';
    ...
    if (sku) {
      ...
      if (nrd && nrd.products) {
        try {
          showSpinner('Verificando SKU...');
          const allProducts = await nrd.products.getAll();
          // Handle both array and object formats
          const productsArray = Array.isArray(allProducts) 
            ? allProducts 
            : Object.entries(allProducts || {}).map(([id, product]) => ({ id, ...product }));
          
          // Check if SKU already exists in another product
          const duplicateProduct = productsArray.find(p => {
            if (!p) return false;
            
            // Get product ID (can be id, productId, or key from object)
            const pId = p.id || p.productId;
            
            // Skip current product if updating
            if (productId && pId === productId) {
              return false;
            }
            
            // Check if SKU matches (case-insensitive and trimmed)
            return p.sku && p.sku.trim().toLowerCase() === sku.trim().toLowerCase();
          });
          
          hideSpinner();
          
          if (duplicateProduct) {
            await showError(`El SKU "${sku}" ya está en uso por otro producto: "${duplicateProduct.name || 'Sin nombre'}"`);
            return;
          }
        } catch (error) {
          hideSpinner();
          logger.error('Error validating SKU uniqueness', error);
          await showError('Error al verificar el SKU. Por favor, intente nuevamente.');
          return;
        }
      }
    }
    ...
    try {
      ...
      if (variants.length > 0) {
        ...
        // Si tiene variantes, el producto padre no es vendible
        productData.esVendible = false;
      } else {
        // Si no tiene variantes, puede ser vendible según el checkbox
      ...
      if (unidadInsumo) {
        productData.unidadInsumo = unidadInsumo;
      }
      if (conversionesArray.length > 0) {
        productData.conversiones = conversionesArray;
      }
      const tags = getProductTagsFromForm();
      productData.tags = tags;
      const supplierIdVal = document.getElementById('product-supplier')?.value?.trim();
      const supplierCodeVal = document.getElementById('product-supplier-code')?.value?.trim();
      if (supplierIdVal) productData.supplierId = supplierIdVal;
      else productData.supplierId = undefined;
      productData.supplierCode = supplierCodeVal || undefined;
    ...
    messageEl.innerHTML = `
      <div class="space-y-4">
        ...
        <div class="pt-2 border-t border-gray-200">
          <p class="text-xs uppercase tracking-wider text-gray-600 mb-2">Valores fijos (opcional)</p>
          <p class="text-xs text-gray-500 mb-2">Estos valores se aplicarán a todos los productos importados.</p>
          <div class="flex flex-wrap gap-4">
            <label class="flex items-center gap-2">
              <input type="checkbox" id="import-fixed-active" checked class="w-4 h-4 border-gray-300 rounded text-red-600 focus:ring-red-600">
              <span class="text-sm text-gray-700">Activo</span>
            </label>
            <label class="flex items-center gap-2">
              <input type="checkbox" id="import-fixed-es-vendible" checked class="w-4 h-4 border-gray-300 rounded text-red-600 focus:ring-red-600">
              <span class="text-sm text-gray-700">Es vendible</span>
            </label>
            <label class="flex items-center gap-2">
              <input type="checkbox" id="import-fixed-es-producible" class="w-4 h-4 border-gray-300 rounded text-red-600 focus:ring-red-600">
              <span class="text-sm text-gray-700">Es producible</span>
            </label>
            <label class="flex items-center gap-2">
              <input type="checkbox" id="import-fixed-es-comprable" class="w-4 h-4 border-gray-300 rounded text-red-600 focus:ring-red-600">
              <span class="text-sm text-gray-700">Es comprable</span>
            </label>
          </div>
        </div>
      </div>
    ...
    const handleConfirm = async () => {
      ...
      try {
        ...
        if (!file) {
          ...
          reEnableInFinally = false;
          setTimeout(() => { confirmBtn.disabled = false; }, 400);
          return;
        }

        // Store file reference and fixed values before showing preview (inputs will be removed from DOM)
        const selectedFile = file;
        const fixedValues = {
          active: document.getElementById('import-fixed-active')?.checked !== false,
          esVendible: document.getElementById('import-fixed-es-vendible')?.checked !== false,
          esProducible: document.getElementById('import-fixed-es-producible')?.checked === true,
          esComprable: document.getElementById('import-fixed-es-comprable')?.checked === true
        };
        ...
        await processCSVFile(selectedFile, preview, fixedValues);
        resolve();
      } catch (error) {
        ...
        if (reEnableInFinally) confirmBtn.disabled = false;
      }
    };
...
async function previewCSVFile(file) {
  try {
    ...
    const normalizeText = (text) => {
      if (!text) return '';
      // Remove accents/diacritics, special characters, spaces, and convert to lowercase
      return text
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '') // Remove diacritics (accents)
        .replace(/[^\w]/g, '') // Remove all non-word characters (spaces, special chars, etc.)
        .toLowerCase()
        .trim();
    };
    ...
    const findColumnIndex = (normalizedHeader, patterns, excludePatterns = []) => {
      for (const pattern of patterns) {
        const index = normalizedHeader.findIndex((h, idx) => {
          // Skip if this column matches any exclude pattern (to avoid false positives)
          if (excludePatterns.some(exclude => {
            const excludeNoSpaces = exclude.replace(/\s+/g, '');
            return h.includes(excludeNoSpaces);
          })) {
            return false;
          }
          
          // Remove spaces from header for comparison
          const hClean = h.replace(/\s+/g, '');
          const patternClean = pattern.replace(/\s+/g, '');
          
          // Exact match (highest priority)
          if (hClean === patternClean) return true;
          // Starts with pattern (at least 3 chars for reliability)
          if (hClean.startsWith(patternClean) && patternClean.length >= 3) return true;
          // Contains pattern (for partial matches)
          if (hClean.includes(patternClean) && patternClean.length >= 3) return true;
          // Match without vowels for encoding issues (e.g., "cdigo" matches "codigo")
          const hNoVowels = hClean.replace(/[aeiou]/g, '');
          const patternNoVowels = patternClean.replace(/[aeiou]/g, '');
    ...
    // Use excludePatterns to avoid false positives (e.g., don't match "contado" when looking for "codigo")
    // Patterns ordered by specificity (most specific first)
    const codigoIndex = findColumnIndex(normalizedHeader, ['codigo', 'cdigo', 'cod', 'cdig'], ['contado', 'articulo', 'precio', 'art']);
    const articuloIndex = findColumnIndex(normalizedHeader, ['articulo', 'artculo', 'art', 'artcul'], ['codigo', 'contado', 'precio', 'cod']);
    const contadoIndex = findColumnIndex(normalizedHeader, ['contado', 'contad', 'precio', 'prec'], ['codigo', 'articulo', 'art', 'cod']);
    ...
    console.log('CSV columns validated successfully', {
      ...
      allColumns: header,
      allNormalized: normalizedHeader
    });
    ...
    // getAll() sin params: estructura completa para import (variants anidados)
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      // Debug: log column indices and values for first few lines
      if (i <= 3) {
        console.log(`Processing line ${i + 1}:`, {
          ...
          articuloIndex,
          contadoIndex,
          codigoValue: columns[codigoIndex],
          articuloValue: columns[articuloIndex],
          contadoValue: columns[contadoIndex]
        });
      }
      
      // SKU is a complete string (e.g., "P0965") - preserve as string, don't convert
      const sku = columns[codigoIndex] !== undefined && columns[codigoIndex] !== null 
        ? String(columns[codigoIndex]).trim() 
        : '';
      const name = columns[articuloIndex] !== undefined && columns[articuloIndex] !== null
        ? String(columns[articuloIndex]).trim() 
        : '';
      const priceStr = columns[contadoIndex] !== undefined && columns[contadoIndex] !== null
        ? String(columns[contadoIndex]).trim() 
        : '';
      ...
      // Ensure SKU is preserved as string (e.g., "P0965" should remain "P0965", not converted to number)
      // Log for debugging
      if (i <= 3) { // Log first few SKUs for debugging
        console.log(`SKU detected (line ${i + 1}):`, { 
          codigoIndex,
          allColumns: columns,
          codigoColumnValue: columns[codigoIndex],
          skuValue: sku,
          type: typeof sku,
          length: sku.length 
        });
      ...
      // Ensure SKU is stored as string (preserve complete SKU like "P0965")
      products.push({
        sku: String(sku), // Explicitly convert to string to preserve format
        name: String(name),
        price: parseFloat(price), // Price is numeric
        exists,
        priceEqual
      });
    }
    ...
    let previewHTML = `
      <div class="space-y-4">
        <div class="text-sm text-gray-600">
          ...
          <p><strong>Actualizaciones (nombre y precio):</strong> <span class="text-green-600">${preview.toUpdateEqual + preview.toUpdateDifferent}</span></p>
          <p class="text-xs text-gray-500">Los productos existentes se actualizarán con el nombre y precio del CSV</p>
        </div>
    `;
...
// Process CSV file (actual import). fixedValues: { active, esVendible, esProducible, esComprable }
async function processCSVFile(file, preview, fixedValues = {}) {
  const nrd = window.nrd;
  if (!nrd) {
    await (window.showError || alert)('Servicio no disponible');
    return;
  ...
  try {
    ...
    const applyFixed = (base) => {
      const out = { ...base };
      if (fixedValues.active !== undefined) out.active = fixedValues.active;
      if (fixedValues.esVendible !== undefined) out.esVendible = fixedValues.esVendible;
      if (fixedValues.esProducible !== undefined) out.esProducible = fixedValues.esProducible;
      if (fixedValues.esComprable !== undefined) out.esComprable = fixedValues.esComprable;
      return out;
    };
    ...
    for (const product of preview.products) {
      try {
        if (product.exists) {
          const updates = applyFixed({
            name: product.name,
            price: product.price,
            esVendible: true
          });
          await nrd.products.update(existingProductsMap[product.sku].id, updates);
          updated++;
        } else {
          const createData = applyFixed({
            sku: product.sku,
            name: product.name,
            price: product.price,
            active: true,
          ...
          await nrd.products.create(createData);
          added++;
        }
      } catch (error) {
...
// Detect if UTF-8 decoded text looks like mojibake (e.g. Ã¡ instead of á)
function looksLikeMojibake(text) {
  if (!text || text.length < 2) return false;
  return /Ã[¡¢£¤¥¦§¨©ª«¬­®¯°±²³´µ¶·¸¹º»¼½¾¿ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿ]/.test(text) || /Ã./.test(text);
}

// Read file as text; try UTF-8 first, fallback to Windows-1252 if tildes/special chars look wrong
...
/**
 * Load all unique tags from products (for advanced filter modal)
 */
async function loadAllProductTagsForFilter() {
  const nrd = window.nrd;
  if (!nrd || !nrd.products) return [];
  try {
    const flat = await nrd.products.getAll({ flat: true });
    const items = Array.isArray(flat) ? flat : Object.values(flat || {});
    const set = new Set();
    items.forEach(item => {
      normalizeTagsFromProduct(item).forEach(t => {
        const s = String(t).trim();
    ...
    logger.warn('Error loading tags for filter', e);
    return [];
  }
}

/**
 * Setup advanced filters modal and handlers
 */
function setupAdvancedFilters() {
  const openBtn = document.getElementById('products-advanced-filter-btn');
  const modal = document.getElementById('advanced-filters-modal');
  const closeBtn = document.getElementById('close-advanced-filters-modal');
  const searchBtn = document.getElementById('advanced-filters-search-btn');
  const clearBtn = document.getElementById('advanced-filters-clear-btn');
  const variantsSinRadio = document.getElementById('filter-variants-sin');
  const variantsConRadio = document.getElementById('filter-variants-con');
  const variantsTodosRadio = document.getElementById('filter-variants-todos');
  const tagsContainer = document.getElementById('advanced-filters-tags-container');
  const filterCheckboxes = {
    vendible: document.getElementById('filter-role-vendible'),
    comprable: document.getElementById('filter-role-comprable'),
    insumo: document.getElementById('filter-role-insumo'),
    producible: document.getElementById('filter-role-producible')
  };
  
  if (!openBtn || !modal) return;
  
  openBtn.addEventListener('click', async () => {
    if (variantsSinRadio) variantsSinRadio.checked = productsVariantsMode === 'sin';
    if (variantsConRadio) variantsConRadio.checked = productsVariantsMode === 'con';
    if (variantsTodosRadio) variantsTodosRadio.checked = productsVariantsMode === 'todos';
    if (filterCheckboxes.vendible) filterCheckboxes.vendible.checked = productsRoleFilter.includes('vendible');
    if (filterCheckboxes.comprable) filterCheckboxes.comprable.checked = productsRoleFilter.includes('comprable');
    if (filterCheckboxes.insumo) filterCheckboxes.insumo.checked = productsRoleFilter.includes('insumo');
    if (filterCheckboxes.producible) filterCheckboxes.producible.checked = productsRoleFilter.includes('producible');
    if (tagsContainer) {
      tagsContainer.innerHTML = '<div class="text-xs text-gray-500 py-2">Cargando etiquetas...</div>';
      const allTags = await loadAllProductTagsForFilter();
      if (allTags.length === 0) {
        tagsContainer.innerHTML = '<div class="text-xs text-gray-500 py-2">No hay etiquetas</div>';
      } else {
        tagsContainer.innerHTML = allTags.map(tag => {
          const checked = productsTagsFilter.includes(tag) ? ' checked' : '';
          return `<label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" class="filter-tag-cb rounded border-gray-300 text-red-600 focus:ring-red-600" data-tag="${escapeHtml(tag)}"${checked}><span class="text-sm">${escapeHtml(tag)}</span></label>`;
        }).join('');
      }
    }
    modal.classList.remove('hidden');
  ...
  if (searchBtn) {
    searchBtn.addEventListener('click', () => {
      if (variantsSinRadio?.checked) productsVariantsMode = 'sin';
      else if (variantsConRadio?.checked) productsVariantsMode = 'con';
      else if (variantsTodosRadio?.checked) productsVariantsMode = 'todos';
      else productsVariantsMode = 'todos';
      productsRoleFilter = [];
      if (filterCheckboxes.vendible?.checked) productsRoleFilter.push('vendible');
      if (filterCheckboxes.comprable?.checked) productsRoleFilter.push('comprable');
      if (filterCheckboxes.insumo?.checked) productsRoleFilter.push('insumo');
      if (filterCheckboxes.producible?.checked) productsRoleFilter.push('producible');
      productsTagsFilter = [];
      if (tagsContainer) {
        tagsContainer.querySelectorAll('.filter-tag-cb:checked').forEach(cb => {
          const tag = cb.dataset.tag;
          if (tag) productsTagsFilter.push(tag);
        });
      }
      updateFilterBadge();
      loadProducts();
  ...
  if (clearBtn) {
    clearBtn.addEventListener('click', () => {
      productsVariantsMode = 'todos';
      productsRoleFilter = [];
      productsTagsFilter = [];
      if (variantsSinRadio) variantsSinRadio.checked = false;
      if (variantsConRadio) variantsConRadio.checked = false;
      if (variantsTodosRadio) variantsTodosRadio.checked = true;
      if (filterCheckboxes.vendible) filterCheckboxes.vendible.checked = false;
      if (filterCheckboxes.comprable) filterCheckboxes.comprable.checked = false;
      if (filterCheckboxes.insumo) filterCheckboxes.insumo.checked = false;
      if (filterCheckboxes.producible) filterCheckboxes.producible.checked = false;
      if (tagsContainer) {
        tagsContainer.querySelectorAll('.filter-tag-cb').forEach(cb => { cb.checked = false; });
      }
      updateFilterBadge();
      loadProducts();
      closeModal();
...
function updateFilterBadge() {
  const badge = document.getElementById('products-filter-badge');
  if (!badge) return;
  const hasVariantsFilter = productsVariantsMode !== 'todos';
  const count = productsRoleFilter.length + productsTagsFilter.length + (hasVariantsFilter ? 1 : 0);
  if (count > 0) {
...
export function initializeProducts() {
  ...
  const nrd = window.nrd;
  if (nrd && nrd.products) {
    productsListener = nrd.products.onValue(() => loadProducts());
  }
